...
function AddEditBookForm({type,book}) {
  ...
  const addBook = (e) =>{
    ...
    if(type === "Add"){
      ...
      axios.patch(`http://localhost:4000/api/books/${book._id}`, formDataObject)
      .then((response) => {
        console.log('Request was successful:', response.data);
      })
      .catch((error) => {
    ...
    <>
      <div className={style.fromContainer}>
        <form className={style.bookform} id="bookForm"onSubmit={addBook}>
          <h1 className={style.title}>Launch book</h1>
          <div className={style.inputContainer}>
            <label className={style.label}>Title</label>
            <input className={style.input} type="text" placeholder="Enter Title" name="title" defaultValue={type === 'Edit' ? book.title : ''} required/>
          </div>
          <div className={style.inputContainer}>
            <label>Pages</label>
            <input className={style.input} type="number" placeholder="Enter the pages" name="nbPages" defaultValue={type === 'Edit' ? book.nbPages : ''} required/>
          </div>
          <div className={style.inputContainer}>
            ...
            className={style.input}
              ...
              defaultValue={type === 'Edit' ? formatDate(book.publicationDate)  : ''}
              required
            />
          </div>
          <div className={style.inputContainer}>
            <label className={style.label}>ISBN</label>
            <input className={style.input} type="string" placeholder="Enter the ISBN" name="ISBN" defaultValue={type === 'Edit' ? book.ISBN : ''} required/>
          </div>
          <div className={style.inputContainer}>
          ...
          <div className={style.inputContainer}>
            ...
            <select name="authorId" defaultValue={type === 'Edit' ? book._id : ''} required>
          ...
          <div className={style.inputContainer}>
            ...
            <select name="categoryId" defaultValue={type === 'Edit' ? book.categoryId : ''} required>
          ...
          <div className={style.inputContainer}>
            ...
            <select name="language" defaultValue={type === 'Edit' ? book.language : ''} required>
          ...
          <div className={style.inputContainer}>
            ...
            <input className={style.input} type="file" name="image"  required={type==="Add"}/>
          </div>
          <div className={style.buttonContainer}>
            <button className={style.cancel}>Cancel</button>
            <button className={style.add}>{type === 'Add' ? 'Add' : 'Edit'}</button>
          </div>
        </form>
      </div>
    </>
